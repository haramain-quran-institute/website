...
import { ArrowLeft, BadgePercent, CalendarDays, WalletCards } from "lucide-react";

import heroImage from "@/assets/CoursesFee-Images/hero.webp";

export default function CoursesFeeHero() {
  return (
    <section className="relative flex min-h-[650px] items-end overflow-hidden pb-20 pt-40 text-white min-[768px]:min-h-[720px] min-[1024px]:pb-24">
      <Image
        src={heroImage}
        alt="Haramain Quran Institute courses fee and schedule"
        fill
        priority
        sizes="100vw"
      ...
      <div className="container relative z-10">
        <div className="max-w-4xl">
          ...
          <div className="mt-8 flex flex-wrap gap-x-7 gap-y-3 text-sm text-white/80">
            <span className="inline-flex items-center gap-2"><WalletCards className="size-4 text-[#D0A86C]" /> Two clear fee groups</span>
            <span className="inline-flex items-center gap-2"><CalendarDays className="size-4 text-[#D0A86C]" /> Five weekly plans</span>
            <span className="inline-flex items-center gap-2"><BadgePercent className="size-4 text-[#D0A86C]" /> 20% yearly saving</span>
          </div>
