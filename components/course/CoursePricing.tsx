...
function getPlans(group: CoursePageData["pricingGroup"]) {
  const standard = group === "foundation";

  return [
    { title: "2 Days", subtitle: "per week", usd: standard ? 30 : 45 },
    { title: "3 Days", subtitle: "per week", usd: standard ? 35 : 50 },
    { title: "4 Days", subtitle: "per week", usd: standard ? 40 : 55 },
    { title: "5 Days", subtitle: "per week", usd: standard ? 45 : 60 },
    { title: "Weekend", subtitle: "2 days plan", usd: standard ? 35 : 50 },
  ];
}
...
export default function CoursePricing({ course }: { course: CoursePageData }) {
  const [currency, setCurrency] = useState<CurrencyCode>("USD");
  const [yearly, setYearly] = useState(false);
  const plans = useMemo(() => getPlans(course.pricingGroup), [course.pricingGroup]);

  return (
    <section id="fee-schedule" className="w-full bg-white py-20 sm:py-24 min-[1024px]:py-32">
      <div className="container">
        <div className="mx-auto max-w-3xl text-center">
          <div className="mx-auto flex size-12 items-center justify-center rounded-full bg-[#0D463E] text-[#FBF6EF]">
            <CircleDollarSign className="size-6" />
          </div>
          <p className="mt-6 font-body text-sm font-semibold uppercase tracking-[0.18em] text-[#B08242]">
            Fee & Schedule
          </p>
          <h2 className="mt-5 font-heading text-4xl font-medium leading-[1.04] tracking-tight text-[#0D463E] sm:text-5xl min-[1024px]:text-[58px]">
            Choose a plan that fits your <span className="font-accent font-normal italic">routine.</span>
          </h2>
          <p className="mx-auto mt-6 max-w-2xl font-body text-base leading-7 text-[#0D463E]/65">
            Simple monthly pricing for {course.title}, with flexible timings and 20% automatically deducted when you select yearly billing.
          </p>
        </div>

        <div className="mt-10 flex flex-col items-center justify-center gap-5 min-[760px]:flex-row min-[760px]:gap-8">
          <div className="inline-flex rounded-full border border-[#0D463E]/10 bg-white p-1" aria-label="Select currency">
            {(Object.keys(currencies) as CurrencyCode[]).map((code) => (
              <button
                ...
                className={`rounded-full px-4 py-2 font-body text-xs font-semibold transition-colors sm:px-5 ${
          ...
          <div className="flex flex-wrap items-center justify-center gap-3" aria-label="Billing period">
            <button
              ...
              className={`font-body text-sm font-semibold transition-colors ${!yearly ? "text-[#0D463E]" : "text-[#0D463E]/45 hover:text-[#0D463E]"}`}
            >
              Monthly
            </button>
            <button
        ...
        <div className="mt-12 grid grid-cols-1 gap-4 min-[620px]:grid-cols-2 min-[900px]:grid-cols-3 min-[1200px]:grid-cols-5">
        ...
        <p className="mx-auto mt-8 max-w-3xl text-center font-body text-xs leading-6 text-[#0D463E]/50">
          Prices are shown per student, per month. Currency conversion is provided for convenient display; your enrollment confirmation will include the final payable amount.
        </p>
      </div>
    </section>
  );
