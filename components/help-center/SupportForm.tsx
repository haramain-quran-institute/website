...
const inputClass =
  "mt-2 h-12 w-full rounded-[var(--radius-sm)] border border-[#0D463E]/15 bg-white px-4 font-body font-normal text-[#0D463E] outline-none transition-colors placeholder:text-[#0D463E]/35 focus:border-[#0D706D]";
  ...
  return (
    <section id="support-form" className="bg-[#0D463E] py-24 text-white sm:py-28 min-[1024px]:py-32">
      <div className="container grid gap-12 min-[960px]:grid-cols-[0.82fr_1.18fr] min-[960px]:items-center min-[1100px]:gap-20">
        ...
        <div className="rounded-[var(--radius-lg)] bg-[#FBF6EF] p-6 text-[#0D463E] shadow-2xl sm:p-9">
          {success ? (
            <div className="flex min-h-[430px] flex-col items-center justify-center text-center">
              ...
              <button type="button" onClick={() => setSuccess(false)} className="mt-7 rounded-[var(--radius-sm)] border border-[#0D463E]/20 px-5 py-3 font-body text-sm font-bold hover:bg-white">Send another message</button>
            </div>
          ) : (
            <form onSubmit={submit} className="space-y-5">
              ...
              <fieldset>
                ...
                <div className="mt-2 grid grid-cols-3 gap-2">
                  {(["Email", "Phone", "WhatsApp"] as const).map((method) => (
                    <button key={method} type="button" onClick={() => setPreferredReply(method)} aria-pressed={preferredReply === method} className={`h-11 rounded-[var(--radius-sm)] border px-2 font-body text-xs font-bold transition-colors sm:text-sm ${preferredReply === method ? "border-[#0D463E] bg-[#0D463E] text-white" : "border-[#0D463E]/15 bg-white text-[#0D463E] hover:border-[#0D463E]/35"}`}>{method}</button>
                  ))}
                </div>
              </fieldset>
              ...
              {error && <p role="alert" className="rounded-[var(--radius-sm)] bg-red-50 px-4 py-3 font-body text-sm leading-5 text-red-700">{error}</p>}
              <button disabled={pending} type="submit" className="inline-flex h-13 w-full items-center justify-center gap-2 rounded-[var(--radius-sm)] bg-[#0D463E] px-6 font-body text-sm font-bold text-white transition-colors hover:bg-[#146154] disabled:cursor-not-allowed disabled:opacity-65">
