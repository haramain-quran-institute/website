...
import { yesNoOptions, firstExperienceOptions } from "./schemas/common";
import { useHajjPageForm } from "./hooks/use-hajj-page-form";
import {
  FormInput,
  FormTextarea,
  FormSelect,
...
const hajjDurationOptions = [
  { value: "9-days", label: "9 Days" },
  { value: "11-days", label: "11 Days" },
  { value: "19-days", label: "19 Days" },
];

export function HajjPageForm({
  payload,
}: {
  payload?: Record<string, unknown>;
}) {
  const { form, status, errorMessage, onSubmit } = useHajjPageForm();
  ...
  return (
    <div className="mx-auto flex max-w-4xl flex-col gap-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <FormField
            label="Did you have performed Hajj before?"
        ...
        <FormField
          label="9 Days, 11 Days or 19 Days"
          htmlFor="duration"
          error={errors.duration?.message}
        >
          <FormSelect
            id="duration"
            options={hajjDurationOptions}
        ...
        >
          <span className="z-10">
            {isSubmitting ? "Sending..." : "Send Hajj Inquiry"}
          </span>
        </Button>
      </form>
    </div>
